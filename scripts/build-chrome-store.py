...
def make_promo(width: int, height: int, path: Path) -> None:
    ...
    draw_text(pixels, width, height, "ONE CHUTE MANY BROWSERS", int(width * 0.40), int(height * 0.58), scale, cream)
...
def main() -> int:
    ...
    print(f"Chrome Web Store ZIP: {zip_base.with_suffix('.zip')}")
    ...
    print("Still required in the Developer Dashboard: at least one real 1280x800 extension screenshot.")
    return 0
